...
function breadcrumbs_plus( $args = '' ) {
	...
	$defaults = array(
		'prefix' => '<p class="breadcrumbs">',
		'suffix' => '</p>',
		'title' => __( 'You are here: ', ThemeSetup::HOOK . '_theme' ),
		'home' => __( 'Home', ThemeSetup::HOOK . '_theme' ),
		'sep' => '&raquo;',
		'front_page' => false,
		'bold' => false,
		'show_blog' => true,
	...
	$home = '<a href="'. home_url( '/' ) .'" class="breadcrumbs-begin">' . $args['home'] . '</a>';
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_date() ) {
			...
			if ( is_day() )
				$html .= _bold_( __( 'Archives for ', ThemeSetup::HOOK . '_theme' ) . get_the_time( 'F j, Y' ), $args['bold'] );
			
			elseif ( is_month() )
				$html .= _bold_( __( 'Archives for ', ThemeSetup::HOOK . '_theme' ) . single_month_title( ' ', false ), $args['bold'] );
					
			elseif ( is_year() )
				$html .= _bold_( __( 'Archives for ', ThemeSetup::HOOK . '_theme' ) . get_the_time( 'Y' ), $args['bold'] );
		}

		elseif ( is_author() )
			$html .= _bold_( __( 'Author / ', ThemeSetup::HOOK . '_theme' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author ), $args['bold'] );
	}

	/* If viewing search results. */
	elseif ( is_search() )
		$html .= $home . $separator . _bold_( __( 'Search results for: ', ThemeSetup::HOOK . '_theme' ) . stripslashes( strip_tags( get_search_query() ) ) . '', $args['bold'] );

	/* If viewing a 404 error page. */
	elseif ( is_404() )
		$html .= $home . $separator . _bold_( __( 'Page Not Found', ThemeSetup::HOOK . '_theme' ), $args['bold'] );

...
function eps_add_input(){
    global $post_ID, $post;
    if(!in_array($post->post_type, array('mlsgallery', 'post', 'page'))){ return;}
...
function eps_set_subtitle( $post_id ){
    ...
    if(is_object($post) && !in_array($post->post_type, array('mlsgallery', 'post', 'page'))){ return;}
    if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) return $post_id;
    
    if(!isset($_POST['eps_subtitle'])) $_POST['eps_subtitle'] = ''; 
    
    update_post_meta( $post_id, "kids_eps_subtitle", strip_tags(trim($_POST['eps_subtitle']))); 
}
...
function the_subtitle( $pid = null ) {
	...
	$metaData = get_post_meta($postId, "kids_eps_subtitle", true);
...
function eps_admin_css(){
    global $post;
    if(is_object($post) && !in_array($post->post_type, array('mlsgallery', 'post', 'page'))){ return;}
    wp_enqueue_style( 'eps_style' , KIDS_PLUGIN_URI . '/easy-post-subtitle/include/style.css');
}
...
function eps_admin_scripts(){
   ...
   if(is_object($post) && !in_array($post->post_type, array('mlsgallery', 'post', 'page'))){ return;}
   wp_enqueue_script( 'eps_scripts' , KIDS_PLUGIN_URI . '/easy-post-subtitle/include/scripts.js');

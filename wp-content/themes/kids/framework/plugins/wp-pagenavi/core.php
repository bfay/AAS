	...
	switch ( intval( $options['style'] ) ) {
		...
		case 1:
			...
			if ( $paged > 1 && !empty( $options['prev_text'] ) )
				$out .= $instance->get_single( $paged - 1, 'previous-1', $options['prev_text'] );
			...
			foreach ( range( $start_page, $end_page ) as $i ) {
				if ( $i == $paged && !empty( $options['current_text'] ) ) {
					...
					//$out .= "<span class='current'>$current_page_text</span>";
					$out .= $instance->get_single( $i, "current", $options['page_text'] );
					$timeline = 'larger';
				} else {
			...
			if ( $paged < $total_pages && !empty( $options['next_text'] ) )
				$out .= $instance->get_single( $paged + 1, 'next-1', $options['next_text'] );
